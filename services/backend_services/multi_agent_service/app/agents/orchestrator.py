...
from .config import should_use_blackboard, get_multi_agent_config
...
class DesignSession(BaseModel):
    ...
    # Phase 16: Blackboard integration
    use_blackboard: bool = False
    degraded_mode: bool = False
    contract_adapters: Dict[str, Any] = Field(default_factory=dict)
...
class AgentOrchestrator:
    ...
    async def initialize(self) -> None:
        ...
        # Phase 18: Check for agent capability overlap
        self._check_agent_overlap()
        
        self._initialized = True
        logger.info("Agent Orchestrator initialized successfully")
    
    def _check_agent_overlap(self) -> None:
        """
        Phase 18: Check for capability overlap between agents.
        
        Logs warnings if significant overlap is detected to help
        prevent routing confusion and redundancy.
        """
        try:
            from .overlap_detector import detect_agent_overlap
            
            warnings = detect_agent_overlap(self._sub_agents)
            ...
            if warnings:
                ...
                if high_severity:
                    logger.warning(
                        f"Detected {len(high_severity)} high-severity agent overlaps. "
                        "Consider consolidating agents with similar capabilities."
                    )
                    for w in high_severity:
                        logger.warning(f"  - {w.agent_a} <-> {w.agent_b}: {w.details}")
                
                if medium_severity:
                    logger.info(
                        f"Detected {len(medium_severity)} medium-severity agent overlaps."
                    )
            else:
                logger.debug("No significant agent capability overlaps detected.")
                
        except ImportError:
            logger.debug("Overlap detector not available, skipping check")
        except Exception as e:
            logger.warning(f"Error checking agent overlap: {e}")
    ...
    def _initialize_contract_adapters(self, session_id: str) -> Dict[str, ContractAdapter]:
        """
        Initialize contract adapters for all agents in a session.
        
        Phase 16: Each agent gets a ContractAdapter that enforces
        behavioral contracts and state machine rules.
        """
        adapters = {}
        
        # Create adapter for coordinator
        adapters["coordinator"] = ContractAdapter("coordinator", session_id)
        
        # Create adapters for all sub-agents
        for agent_name in self._sub_agents.keys():
            adapters[agent_name] = ContractAdapter(agent_name, session_id)
        
        logger.debug(f"Initialized {len(adapters)} contract adapters for session {session_id}")
        return adapters
    ...
    async def create_session(
        ...
        # Phase 16: Determine if this session should use blackboard
        use_blackboard = should_use_blackboard(context.session_id)
        ...
        # Phase 16: Initialize contract adapters for each agent if using blackboard
        if use_blackboard:
            session.contract_adapters = self._initialize_contract_adapters(session.id)
            logger.info(f"Session {session.id} using blackboard with contract enforcement")
        
        self._sessions[session.id] = session
        logger.info(f"Created design session {session.id} for user {user_id} (blackboard: {use_blackboard})")
