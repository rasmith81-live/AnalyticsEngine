"""
Metadata Client using Pub/Sub for Calculation Engine Service.
...
class MetadataClientPubSub:
    ...
    async def connect(self) -> None:
        ...
        logger.info(f"MetadataClientPubSub connected, reply channel: {self._reply_channel}")
    ...
    async def list_definitions(
        ...
        Args:
            kind: Definition kind (e.g., "metric_definition", "entity_definition")
    ...
    async def search_definitions(
        ...
        try:
            result = await self._request(
                request_type="search_definitions",
                payload={
                    "query": query,
                    "kind": kind,
                    "limit": limit
            ...
            logger.warning(f"Failed to search definitions: {e}")
            return []
